...
import { Check, Clock3, Coffee, Play, RotateCcw } from "lucide-react";
...
export function SessionCard({ session, compact = false, featured = false }: { session: StudySession; compact?: boolean; featured?: boolean }) {
  const { exams, completeSession, learningProgress } = useStudy();
  ...
  const started = Boolean(learningProgress[session.id]);
  return <>
    <article className={`session-card ${done ? "session-done" : ""} ${compact ? "session-compact" : ""} ${featured ? "session-featured" : ""}`} style={{ "--subject-color": exam?.color ?? "#61705d" } as React.CSSProperties}>
      ...
      <div className="session-actions">{done ? <span className="done-label"><Check size={16} />Erledigt</span> : <><Link className={`button ${featured ? "button-primary" : "button-secondary"}`} href={`/learn/${session.id}`}><Play size={15} />{started ? "Fortsetzen" : featured ? "Lernmodus starten" : "Starten"}</Link><button className="session-complete-short" aria-label="Direkt als erledigt markieren" title="Direkt als erledigt markieren" onClick={() => setFeedbackOpen(true)}><Check size={15} /></button></>}</div>
    </article>
