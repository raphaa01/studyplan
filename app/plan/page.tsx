...
import { CalendarPlus, ChevronLeft, ChevronRight, ListTodo, MapPin, RefreshCw } from "lucide-react";
...
export default function PlanPage() {
  const { plan, exams, calendarItems, optimizePlan, completeCalendarItem, removeCalendarItem } = useStudy();
  ...
  return <>
    ...
    <section className="day-detail-panel"><div className="day-detail-head"><div><p className="eyebrow">{formatGermanDate(selectedDay, { weekday: "long" })}</p><h2>{formatGermanDate(selectedDay, { day: "numeric", month: "long" })}</h2></div><span>{minutesLabel(dayMinutes)} Lernzeit · {selectedItems.length} eigene Einträge</span></div>
      <div className="day-detail-list">{[...selectedSessions.map((session) => ({ time: session.startTime, node: <SessionCard key={session.id} session={session} /> })), ...selectedItems.map((item) => ({ time: item.startTime, node: <article key={item.id} className={`personal-agenda-item ${item.status === "completed" ? "done" : ""}`}><div>{item.kind === "appointment" ? <MapPin size={16} /> : <ListTodo size={16} />}</div><span><small>{item.kind === "appointment" ? "Termin" : "To-do"}</small><strong>{item.title}</strong>{item.notes && <p>{item.notes}</p>}</span><div className="personal-actions">{item.status !== "completed" && <Button variant="secondary" onClick={() => completeCalendarItem(item.id)}>Erledigt</Button>}<button aria-label="Eintrag löschen" onClick={() => removeCalendarItem(item.id)}>Löschen</button></div></article> }))].sort((a, b) => a.time.localeCompare(b.time)).map((entry, index) => <div className="agenda-item" key={`${entry.time}-${index}`}><span className="agenda-time">{entry.time}</span>{entry.node}</div>)}</div>
    </section>
