...
export default function SettingsPage() {
  const { preferences, savePreferences, resetDemo } = useStudy();
  const { account, signOut, updateName } = useAccount();
  const router = useRouter();
  const [draft, setDraft] = useState(preferences);
  function save() { savePreferences(draft); updateName(draft.name); }
  ...
  return <>
    ...
    <div className="settings-layout">
      <section className="settings-card account-settings-card"><div className="settings-card-icon"><UserRound size={20} /></div><div><h2>Dein Konto</h2><p>{account?.email}</p></div><label>Dein Name<input value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} /></label><button className="signout-button" onClick={logout}><LogOut size={15} />Auf diesem Gerät abmelden</button></section>
      ...
      <section className="settings-card data-card"><div className="data-icon"><ShieldCheck size={22} /></div><div><h2>Dein Konto bleibt lokal</h2><p>Anmeldung, Prüfungen und Fortschritt werden nur in diesem Browser gespeichert. Für geräteübergreifende Konten wird später ein Backend benötigt.</p><span><Database size={15} />Getrennter Kontospeicher aktiv</span></div></section>
